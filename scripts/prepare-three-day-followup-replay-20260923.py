"""Create one disposable SQLite backup of the current live DB for route follow-up rehearsal."""
import os
import shutil
import sqlite3
...
WORK = f"{ROOT}/three-day-followup-work.sqlite"

if not os.path.isfile(SOURCE) or os.path.exists(WORK):
    raise SystemExit("Source missing or follow-up work DB already exists")
if shutil.disk_usage(ROOT).free < 4 * 1024**3:
    raise SystemExit("Insufficient space for the follow-up replay")
...
try:
    ...
    if integrity != "ok" or foreign_keys or route[:2] != (4, "qa_failed"):
        raise RuntimeError(f"Follow-up copy invalid: {integrity}, FK={len(foreign_keys)}, route={route}")
finally:
    check.close()
