...
The migration helper requires free space greater than three times the non-backup data size plus 2 GiB. Check that requirement before installing a startup wrapper. Its recovery helper is pinned to schema 76 and Content Strategy 3.8. After rollout, the VM startup metadata must start both `engine` and `engine-worker` from the immutable image digest; preserving an older single-container startup script would make a later VM reboot fail. Keep the latest verified pre-upgrade snapshot and original media intact. If the public API or Worker has processed new data, stop both roles and reconcile those writes before restoring the paired old code/database/media.
