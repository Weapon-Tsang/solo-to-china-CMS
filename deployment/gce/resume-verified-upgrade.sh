...
for ((attempt=0; attempt<60; attempt++)); do
  if docker exec engine node -e 'const r=await fetch("http://127.0.0.1:8080/api/health"); const h=await r.json(); if(!r.ok||h.version!=="2.0.8"||h.contentStrategy.version!=="3.3"||h.captureMediaProtocol.version!==2)process.exit(1)' >"$RELEASE/readiness.log" 2>&1; then
    READY=1; break
  fi
