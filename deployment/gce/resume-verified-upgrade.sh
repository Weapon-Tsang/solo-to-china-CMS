...
assert result['schema']==72 and result['integrity']=='ok'
...
assert db.execute('SELECT MAX(version) FROM schema_migrations').fetchone()[0]==72
db.close()
PY
...
for ((attempt=0; attempt<60; attempt++)); do
  if docker exec --env "EXPECTED_VERSION=$VERSION" engine node -e 'const r=await fetch("http://127.0.0.1:8080/api/health"); const h=await r.json(); if(!r.ok||h.version!==process.env.EXPECTED_VERSION||h.contentStrategy.version!=="3.6"||h.captureMediaProtocol.version!==2)process.exit(1)' >"$RELEASE/readiness.log" 2>&1; then
    READY=1; break
  fi
