...
npm run test:smoke
```
# Content production workbench operations (2.0.13)
...
Before any 2.0.13 deployment, capture a new verified backup and read-only production classification/model-call/active-job baseline, rehearse schema 68→69 offline, and compare the dry-run projection. Do not use reconciliation to enqueue the projected rows. The ignored helper snapshot may be projected with `node scripts/project-production-state-hotfix.mjs <before.json> <after.json>`; it reads JSON and writes no database.
