...
When adding visuals beyond the writer's explicit plan, evaluate relevant saved photographs and analyzed Chinese information graphics together. A qualified original is reused directly; a valuable Chinese infographic, collage or route card is queued for complete English localization and independent visual QA. The article visual limit applies to their combined reader value, not a photograph-only quota. A failed translation QA keeps the slot under review and identifies the missing or mistranslated information; it must not be silently replaced by an easier photograph.
...
Some historical successful transformations have an independent four-field QA receipt and binary SHA-256 but lost their local path or predate the file-hash binding field. A recovery may restore the path and bind the old QA receipt only after reading a stored file with the same SHA-256 and verifying the unchanged frozen visual plan. It must refuse failed QA, changed plans, unknown provider outcomes and missing files. This local recovery does not call a Provider.
