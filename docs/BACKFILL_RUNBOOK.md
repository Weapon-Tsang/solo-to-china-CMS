...
Every mutating processing-gap or recommendation refresh must name the exact dry-run it was reviewed against:

```powershell
npm run backfill -- media --execute
npm run backfill -- experience --execute
npm run backfill -- processing-gaps --execute --approved-from processing_gap_run_...
npm run backfill -- recommendations --execute --approved-from backfill_...
```
...
- `recommendations`: inventories Sources whose diagnostic belongs to an older Content Strategy, protects the inventory with a fingerprint, queues only those diagnostics after the matching dry-run is supplied, then rebuilds topic clusters, Knowledge-event opportunities, coverage and approved readiness without changing human decisions.
...
Before execution, run `npm run backup` and `npm run backup:verify`. After execution, check Settings → Advanced maintenance, queue depth, exception counts and a sample Source/Opportunity. The system keeps one verified local snapshot by default; a new verified snapshot replaces the previous local snapshot. The system does not offer an automatic destructive rollback for backfills; restore the complete versioned snapshot (database and files together) if an operator-approved run must be reversed.
