...
- App/Extension version `1.17.22`, schema migration `37`, and Content Strategy `1.7` are the expected local implementation baseline. Source Adaptation, Topic Feature, and Multi-Source Synthesis are parallel opportunities. Alternative itineraries, viewpoints, location relationships, travel-time estimates, crowd observations, dish lists, photo locations, and ordinary aliases coexist unless same-scope hard facts are genuinely incompatible. Equivalent prices/hours, semantic negations, and metro-station exit aliases are normalized before human review. Claim classifier revisions automatically enqueue a one-time rebuild for destinations that still have pending reviews, so historical false positives disappear without click-by-click intervention. High-volume text/image extraction and text coverage audits use asynchronous Vertex Batch and update original jobs on completion; video, small tails, and failed items retain realtime fallback. Suggested `part_of`, `located_in`, and other non-identity entity relationships are stored as relationships instead of asking an editor to merge distinct entities. Knowledge rebuilds cache Claims and aliases, reuse prepared SQLite statements, skip unchanged Claim/Knowledge writes, and restrict broad/specific generalization to matching canonical predicate/value groups. Startup compares new Claims with the destination's transactionally committed rebuild watermark and queues only stale destinations, so a restart no longer refreshes the whole source queue. The Editorial Assignments workspace lists system topics alongside human-authored assignments, evaluates only topic-scoped evidence, reports city/Attraction-specific acquisition gaps, and requires an explicit queue action before content planning.
