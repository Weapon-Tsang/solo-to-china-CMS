...
When an old downstream failure lacks a prerequisite required by the current registry, it is not allowed to override the live recovery target. `production_state` 1.2 moves its attribution to `latest_historical_error`, annotates the timeline step as non-blocking history, reports the live state as `interrupted`, and chooses the first missing prerequisite-safe step. This is how an old Page Plan failure now resumes Narrative/Packet assembly rather than attempting to skip directly to Page Plan.
