...
## Vertex AI 吞吐与冷却调优（2026-09-09）

- 核对 Google 官方文档后确认：`gemini-3.8-flash` 的 `global` 端点配置正确；标准按量服务使用动态共享容量，没有可保证永不 429 的固定 RPM 或固定冷却秒数。官方建议平滑突发流量，并对 429/暂时过载采用带随机抖动的截断指数退避。
- 版本 1.17.10 将模型请求统一错开 1 秒；AI 队列并发从 2 起步，连续成功 12 次才增加 1 路，最高 4 路；任何提取、诊断、内容规划、写作、审核或配图任务遇到供应商压力后都会立即把并发减半，最低 1 路。
- 429/500/503 供应商压力采用 5 秒起步、最高 5 分钟的共享指数冷却，优先遵守服务端 `Retry-After`，并根据连续压力次数递增；只有 5 次真实模型调用成功后才清除压力状态，内存缓存命中不会误判为供应商恢复。
- 所有调优参数已加入 `.env.example` 和 GCE 启动脚本，生产不再依赖隐藏默认值；Gemini 3 请求同时移除了官方说明会被忽略的 `temperature` 参数。
- 本地电脑运行边界已确认：已经上传到 CMS 的处理队列在 GCE 服务器持续运行，本地电脑可以关机；只有小红书浏览器扩展正在发现/打开/采集笔记，或等待人工完成小红书安全验证时，才需要本地 Chrome 和电脑保持运行。
- 1.17.10 验收：吞吐、共享冷却、恢复窗口、配图限流和请求门专项测试全部通过；`npm test` 210/210 通过；`npm run release:check` 的 39 项强制检查全部通过、0 失败。
- 1.17.10 生产发布：提交 `16908c5` 已推送到 `origin/main`；Cloud Build `803e30d3-66aa-4bbc-aca2-283ea6d198a3` 发布镜像摘要 `sha256:9724f07ed1b318a8d52065024617c05971cb908e8422727294605024dff5d45f`。GCE 启动日志确认部署前 SQLite 备份、镜像更新与容器替换成功；`/api/health` 返回版本 1.17.10、Vertex `gemini-3.8-flash`、前端契约 healthy，`/api/ready` 返回应用与数据库 ready，活动队列已在重启后继续下降。
...
- 版本 1.17.9 补齐人工异常的证据链：异常接口同时返回来源 A/B 的来源身份、原文上下文、Evidence Span 与关联图片元数据；受登录保护的预览接口直接读取采集时保存的图片衍生件，旧记录可回退到 HTTPS 原图。
