...
## Final 2.0.57 deployment and live follow-up

Version 2.0.57 commit `997b8eeefaab74fe0b1d75d78943a739cc99a019` passed 792 local tests, `npm run check`, and the offline release gate (50 mandatory passes, zero failures, five warnings and five untested items). Cloud Build `9d9a9675-0016-4669-a4eb-cf1be2f20d11` produced immutable image SHA-256 `af103be19e84c4a0d84ce94ec60259f4e61ce5eb4413fa0eddf77e160e9daeea`. A code-only switch deployed it to the separate production API and Worker containers, both with `/var/lib/solo-to-china/solo-to-china.sqlite` as the database path. API readiness returned HTTP 200 with version 2.0.57; both containers were running and the VM startup metadata was pinned to the same image and revision. The previous API and Worker are retained as `engine-before-997b8ee` and `engine-worker-before-997b8ee`. The schema and database were not migrated in this switch. The 2.0.55 consistent backup and `stc-pre-2-0-55-visual-20260922` disk snapshot remain the data rollback inputs; a later live-write rollback requires preserving and reconciling those writes first.

The live read-only photo plan returned `media_gate_media_incomplete` for the published post with one missing frozen slot, and `media_gate_media_manifest_missing_or_stale` for the Ciqikou draft. Neither received a delivery job. A fresh WordPress inventory sync Job `job_84e70f4912fe411180177e9f1863c929` succeeded, again recording two `publish` and ten `draft` posts. With current inventory, three historical drafts were eligible for a repair *plan*; the other nine remained blocked by independent QA, manifest, required-media or relevant-source gaps. Eligibility for planning is not delivery readiness.

One bounded live recovery on the Eling article, Job `job_a6e807ac6e12442aa9a61d515531fd9e`, exercised the new post-visual gate. A real `gemini-3.8-flash` source-asset analysis succeeded and classified the original as an editorial infographic. The immutable current-revision manifest still described the earlier `analyze_source_image` slot, so the visual remained planned. The Job failed `MEDIA_INCOMPLETE` instead of falsely succeeding and creating another page-composition Job. The original body, two already selected photographs and WordPress draft were retained. The previous falsely successful visual Job ended at 23:35:18 UTC; the current three visual rows were inserted at 23:35:21 UTC, showing that its image plan appeared too late for that attempt. The exact cause of this ordering remains unresolved. A subsequent dry-run plan would replace existing images and remove a slot; it was not applied without article-relevance review. This live test proves the fail-closed gate, not successful historical image delivery. No blind retry was made for the separate persisted `MEDIA_OUTCOME_UNKNOWN` image.

The post-release 2.0.57 same-VM loopback sample sent 31 sequential requests to each of seven routes (217 total), while the separate production Worker was running but no image transformation was active. All responses were HTTP 200. The measured client event-loop delay P95 was 10.41 ms; this is not the server event-loop delay. The cold request, warm P50/P95 and response bytes are actual observations:

| Route | Cold ms | Warm P50/P95 ms | Bytes |
|---|---:|---:|---:|
| Sources, 20 rows | 468.58 | 77.58 / 79.49 | 48,413 |
| Content, 20 rows | 507.28 | 212.86 / 230.02 | 25,589 |
| Recommendations, 20 rows | 490.89 | 40.46 / 45.81 | 13,601 |
| Knowledge, 20 rows | 103.83 | 62.44 / 65.49 | 8,151 |
| Commercial, 20 rows | 16.50 | 2.69 / 3.79 | 7,835 |
| Settings | 898.97 | 290.65 / 298.79 | 25,457 |
| Dashboard summary | 595.67 | 3.83 / 5.66 | 790 |

The same-snapshot before/after SQL and cold/warm results remain in the earlier table. This live sample is not a controlled before/after comparison and does not cover concurrent active image work or authenticated browser paint. Final risk-tier status: L1 targeted **PASS**; L2 module regression **PASS**; L3 production-copy replay **PASS** for migration, twelve-row receipt repair and gate cases, with full historical repair **NOT TESTED**; L4 browser E2E **PARTIAL** (local flows pass, production session expired); L5 real Provider canary **PARTIAL** (source-image analysis and visual QA reached Provider, native article-bundle Schema acceptance hit 429 before generation); L6 full production-like replay **NOT TESTED**; post-fix exploratory audit **ISSUES FOUND** (historical QA gaps, unknown image outcome, late visual seeding and unfixed article delivery). The 72-item acceptance ledger remains conservative; unverified cases are not counted as passed.

The final production health check found zero running Jobs, API readiness HTTP 200/version 2.0.57, and API, Worker and tunnel containers up. Root filesystem free space was 48 GB after subsequent image builds; the earlier 54 GB figure was the immediate result of deleting disposable historical files. The code image is pinned to `997b8ee`; the later `main` commits change documentation only.
