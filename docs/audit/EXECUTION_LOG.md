...
## 2026-09-11 CMS simplification, diagnostics and deployment

- Task IDs: `UI03 / RECOVERY03 / FLOW04 / MEDIA03 / DEPLOY03`; `requirementsVersion=1.3`; status: `completed`. Final runtime code HEAD is `da2646e` on `codex/audit-v1.3`; this record-only commit follows it.
- Pushed commits: `c850df7` (Chinese diagnostics, in-app confirmation, single content-production view, QA rendering fix, removal of active manual planning, retained authorized media and production safeguards), `1b20f8d` (HTTP/2 tunnel transport), `ff47cce` (pin stable cloudflared), and `da2646e` (1.18.3 private-origin keep-alive hardening).
- Final app/extension `1.18.3`; Content Strategy `2.1`; schema `56`. Full `npm test` passed `794/794`; `npm run check` passed; release gate passed `49` mandatory checks with `0` failures, `5` warnings, and `5` explicitly untested/unconfigured conclusions. Mobile Playwright validation at 390×844 had no horizontal overflow, used an in-app `role=dialog`, and recorded 0 console errors/warnings.
- Cloud Build `bae74cb5-a359-40d8-9d54-fb577e7c01a5` succeeded. Image `asia-east1-docker.pkg.dev/project-4bcb9146-c37b-43b0-b11/solo-to-china/engine:1.18.3`; digest `sha256:8dab7ad1924a54136f3b363358219fbb5c4fbb25162827c13f4e67cec34c0bbf`.
- Deployment target stayed `solo-to-china-engine`, `asia-east1-b`, project `project-4bcb9146-c37b-43b0-b11`, with the existing persistent volume. Serial output recorded the verified pre-upgrade database/content snapshot step at `02:53:29Z`, both containers up, and successful completion at `02:53:47Z`. No source, decision, draft, evidence, or WordPress post was deleted, rewritten, approved, retried, or published by deployment probes.
- Rollout diagnosis separated origin health from connector transport. The new `cloudflared 2026.9.0` image showed degrading Docker-bridge tunnel connections; a same-day upstream issue reports a 2026.9.0 Docker-bridge regression and 2026.8.2 as stable. Production now pins `cloudflare/cloudflared:2026.8.2`, uses HTTP/2, and keeps the private Node origin sockets alive for 120 seconds.
- Final local-exit soak: 10 rounds, 52 HTTP 200 responses across health, ready, auth status, authenticated dashboard, authenticated exception workspace, and two large authenticated content responses. Independent Cloud Build probe `8c87a9f2-1c2f-43bd-ac80-3a43b4a6fe56` succeeded from another network: Engine/capture health 200, ready 200, capture root 404, app 1.18.3, DB ready, Strategy 2.1, Frontend Contract healthy at `f44ce1092ced93dfb47d9b3eae83d0d5e4b97086`.
- Existing GCE Guest Agent logging permission warnings remain non-blocking and were not used as a reason to broaden IAM. Real Chrome Load Unpacked, paid model success for every historical item, production WordPress rendered HTML, crawler/search/ranking/traffic/AI-citation outcomes, and restoration of historically missing image bytes are not claimed.

## 2026-09-11 CMS 2.0 major refactor and production deployment

- Status: completed for implementation, tests, push and deployment. Runtime HEAD `34ca0d8`; app/extension `2.0.0`, Content Strategy `3.0`, Schema `57`. Full 14-part record: `docs/audit/2026-09-11-major-cms-refactor.md`.
- Pushed commits: `481f1bf` lifecycle refactor, `6c3f3e4` runtime backfill CLI, `faff791` Experience job deduplication, `34ca0d8` Vertex structured-output compatibility. Final Cloud Build `01d51701-551e-4dfc-8251-f733d19b36b8` succeeded; digest `sha256:297208e457858e247e6b884ea5a2567182ea0768ad9de1fe92f3de5fae03218d`.
- Final GCE startup serial record: verified persistent-volume snapshot at `07:32:44Z`; engine/cloudflared running and deployment completed at `07:33:07Z`. Engine health/ready 200, version `2.0.0`, Strategy `3.0`, database ready, Frontend Contract healthy. Capture health 200 and dashboard 404.
- Dry-run-first production backfills executed with audit IDs. Media completed: 1,311 scanned, 362 originals server-recovered, 949 browser repair required, 0 unavailable. Recommendations completed: 7 destinations, 340 created/refreshed, 610 total opportunities. Failed-production cleanup completed: 15/15 safe rollback, no WordPress post IDs. Experience: 74 queued; online proof reached 5 succeeded/69 pending/0 terminal failures after the Vertex schema correction.
- Release gate: 49 mandatory checks passed, 0 failures, all 1,198 tests included. Five warnings and five deliberately untested conclusions remain: real Chrome/XHS repair, production WordPress rendered/crawler behavior, Search Console and search/traffic/AI-citation outcomes. No automatic WordPress publication occurred.

## 2026-09-11 Media-repair exception diagnosis and production correction

- Status: completed for diagnosis, implementation, tests, push and deployment. Runtime code HEAD `e0f8db8`; app/extension `2.0.1`, Content Strategy `3.0`, Schema `57`.
- Production read-only diagnosis found exactly 949 job exceptions titled “外部模型服务拒绝了请求”. Every sampled subject was a `source_assets` record from `backfill_media_asset`; the media durability backfill independently reported the same 949 assets as `browser_repair_required`. Their retained Xiaohongshu CDN URLs returned HTTP 403 to server-side recovery. The model was not the failing dependency: the generic HTTP-403 fallback had misclassified remote-media errors as model authorization failures.
- Correction: media recovery jobs and `REMOTE_MEDIA_*` codes are classified before the generic provider-403 rule. Permanent or access-controlled media failures now say “来源原件需要浏览器修复”; retryable media-service failures say “来源原件自动保存没有完成”. Browser-repair-required items remain visible in Exceptions and in the repair manifest until verified original bytes are stored. Failed jobs remain as audit history; only already repaired `ORIGINAL_STORED` assets stop appearing as current exceptions.
- Verification: focused regression suites passed 33/33, `npm run check` passed, `git diff --check` passed, and the final `npm run release:check` passed 49 mandatory checks with 0 failures, 5 warnings and 5 explicitly untested/unconfigured conclusions. Regression coverage verifies the 403/503 distinction, persistent repair visibility, audit-job retention and automatic disappearance only after durable storage.
- Cloud Build `d41098e7-2776-43a5-9984-8e27d56d1627` succeeded. Image `asia-east1-docker.pkg.dev/project-4bcb9146-c37b-43b0-b11/solo-to-china/engine:2.0.1`; digest `sha256:8f75ef68054609e29836f2c357513e8f1afa4727e7ddf7a6b651017731e1b811`.
- Deployment target remained GCE `solo-to-china-engine`, zone `asia-east1-b`, project `project-4bcb9146-c37b-43b0-b11`, with the existing persistent volume. Serial output recorded image pull, the verified database-and-content snapshot step, both containers running and `Deployment completed successfully` at `2026-09-11T09:57:36Z`. Existing Guest Agent Cloud Logging permission warnings remain non-blocking; IAM was not broadened.
- Public verification: Engine health/readiness returned HTTP 200, app `2.0.1`, Strategy `3.0`, database `ready`, Frontend Contract `healthy`; capture health returned 200 and capture dashboard returned 404. Exceptions remain 968 records by operator request: 949 accurately titled browser-media repairs, 11 bounded output-limit failures, 5 retained-source-image failures and 3 other operational failures. There are now 0 exceptions titled “外部模型服务拒绝了请求”. The media report remains completed with 1,311 scanned, 362 originals stored, 949 browser repairs required and 0 unavailable.
- Operator next action: sign in to Xiaohongshu in the browser and use the extension's stored-favorites repair/re-capture flow. This deployment does not claim to reconstruct missing originals or complete a real browser capture; those 949 issues intentionally remain visible until their bytes are durably restored.

## 2026-09-11 Dashboard slow-response loading correction

- Status: completed for diagnosis, implementation, tests, push and deployment. Runtime code HEAD `0314e92`; app/extension `2.0.2`, Content Strategy `3.0`, Schema `57`.
- Production diagnosis reproduced blank Content and Settings workspaces in an authenticated browser while the selected tab remained on its loading skeleton. Under active queue work, the five-second poll started another request before the prior multi-megabyte response completed; each new request invalidated the previous result, so no response could commit and repeated overlap eventually exhausted the Engine/Tunnel path. A VM restart restored the saturated path but did not by itself correct the repeatable page-loading race.
- Correction: overview and selected-view loads now coalesce same-key in-flight work, switching views aborts obsolete work, Content returns only approved opportunities and their production rows, and Settings returns a 100-item exception preview with the complete `exceptionTotal`. No exception, failed job, source, media-repair manifest item, decision, draft or WordPress record was deleted or hidden from its canonical workspace. The browser-repair backlog remains operator-visible until legitimate recapture stores the originals.
- Verification: focused suites passed `18/18`; the complete suite passed `1203/1203`; `npm run check` passed build/static/service-boundary checks; `git diff --check` passed. The release gate passed `49` mandatory checks with `0` failures, `5` warnings and `5` explicitly untested/unconfigured conclusions. The duration warning reflects the full test suite exceeding its 30-second advisory budget, not a product failure.
- Cloud Build `6347ce6e-e5fa-4f32-b5cc-7dc3a6da9e9e` succeeded. Image `asia-east1-docker.pkg.dev/project-4bcb9146-c37b-43b0-b11/solo-to-china/engine:2.0.2`; digest `sha256:52c748bd08690885573bbc3a52cbc06d48d8e2a8e98f523bb493215cbd03999d`.
- Deployment target remained GCE `solo-to-china-engine`, zone `asia-east1-b`, project `project-4bcb9146-c37b-43b0-b11`, with the existing persistent volume. Serial output recorded the verified database-and-content pre-upgrade snapshot, both containers running and `Deployment completed successfully` at `2026-09-11T11:05:59Z`. Existing Guest Agent Cloud Logging permission warnings remain non-blocking; IAM and firewall rules were not broadened.
- Production verification: Engine health/readiness returned HTTP 200 with app `2.0.2`, database `ready`, Strategy `3.0` and Frontend Contract `healthy`; capture health returned 200 and capture dashboard returned 404. Eight subsequent public health requests all returned 200 in 0.73-1.37 seconds. In the authenticated browser, Content rendered all 6 approved rows after a 15-second, three-poll observation window; Settings likewise remained rendered after 15 seconds and accurately displayed the full 971-item exception total while receiving only the bounded preview.

## 2026-09-12 CMS 2.0.3 second-pass reconciliation and production deployment

- Status: completed for implementation, tests, push, migration and deployment. Runtime code HEAD `1739675`; app/extension `2.0.3`, Content Strategy `3.0`, Schema `58`. Full evidence: `docs/audit/2026-09-12-cms-second-pass-fixes.md`.
- Final Cloud Build `5529c54a-bde9-4a40-b52e-7b8cda226e79` succeeded. Image digest: `sha256:bcab141f409bbe353962f34f5e0dab9381c827be6e82d4c80c37185fd1bbda3e`.
- Final GCE serial record shows the verified pre-upgrade database/content snapshot step, both containers Up, and deployment completion at `2026-09-11T18:02:11Z`. Engine health/readiness and Capture health returned 200; Capture root remained 404.
- Production semantics now report 822 internal opportunities but 0 actionable recommendations because all 822 have current processing gaps; Content shows 2 actual production entries instead of 6 approved placeholders; System Health shows 6 infrastructure faults instead of 999 broad exception groups. Existing sources, Claims, Knowledge and historical records were preserved.
- Four dry-runs completed with no paid model call or deletion: media `media_backfill_de7ee30db43b426e8576049106329e74`, Experience `backfill_8fd7ee768c7f4271ba2380bea9d41853`, recommendations `backfill_e2efded1410a44b292335619040b797c`, failure cleanup `backfill_9fd52425ead2427ea44747e391b3a3c7`. The 949 missing originals require browser repair; 72 Experience sources remain correctly blocked behind that prerequisite.
- Not verified and not claimed: a real Xiaohongshu extension recapture, successful repair of the 949 historical missing-original items, production WordPress rendered/crawler behavior, Search Console outcomes, or ranking/indexing/traffic/AI-citation changes.

## 2026-09-12 Extension repair synchronization, progress copy and production deployment

- Status: completed for implementation, validation, push and GCE deployment. Runtime code HEAD `54849a6`; app/extension `2.0.5`, Content Strategy `3.0`, Schema `59`.
- Pushed commits: `c540489` (CMS read-path and visual-provenance hardening), `16c3982` (self-healing extension repair synchronization), and `54849a6` (repair progress semantics and copy). The final popup treats `stats.repair` as the cumulative repair total and independently displays completed, in-flight, queued, retry-wait and failed counts; Incremental, Repair and Full use distinct terminology.
- Validation: `npm run check` passed; the complete suite passed `1231/1231`; `npm run release:check` passed 50 mandatory checks with 0 failures, 5 warnings and 5 explicitly untested/unconfigured conclusions. The new popup progress suite passed 4/4 and the extension group passed 28/28.
- Cloud Build `81e58fd9-0a17-4f9e-8959-ba83e7aa59da` succeeded. Image `asia-east1-docker.pkg.dev/project-4bcb9146-c37b-43b0-b11/solo-to-china/engine:2.0.5`; digest `sha256:1c51f03fc7f5f3e64f841aaa4931ec18cfdf0541109d92f9e355193d61d5eb63`.
- Deployment target remained `solo-to-china-engine`, zone `asia-east1-b`, project `project-4bcb9146-c37b-43b0-b11`. Startup created the verified persistent-volume database-and-content snapshot, brought both `engine` and `cloudflared` up, and completed successfully at `2026-09-11T21:56:32Z`.
- Public Engine health/readiness and Capture health returned HTTP 200. Runtime reported version `2.0.5`, Strategy `3.0`, database `ready`, and Frontend Contract `healthy`; Capture dashboard returned 404. Existing background work remained active (`queueActive=281`, `vertexBatchActive=1`) and was not interrupted as a deployment failure.
- The known GCE Guest Agent Cloud Logging permission warning remained non-blocking and IAM was not broadened. The updated extension source was pushed but a real Chrome Load Unpacked/Reload and a real authorized Xiaohongshu repair session remain operator actions; GCE deployment does not publish a Chrome extension or claim that browser-only verification occurred.
