...
test('deployment probe rehearses schema 59 to 72, preserves cited IDs and rolls back paired DB without deleting originals', async t => {
  ...
  assert.equal(db.prepare('SELECT MAX(version) AS v FROM schema_migrations').get().v, 72);
...
test('deployment opportunity gate reconciles deterministically and rejects an actionable row below admission quality', async t => {
  ...
  db.prepare(`INSERT INTO content_opportunities(id,destination_slug,destination_scopes_json,topic_key,strategy_version,title,
    ...
    VALUES ('production-opportunity','production-destination','["production-destination"]','production:topic','3.6','Approved production opportunity',
  ...
  db.prepare(`INSERT INTO content_opportunities(id,destination_slug,destination_scopes_json,topic_key,strategy_version,title,
    ...
    VALUES ('bad-opportunity','bad-destination','["bad-destination"]','bad:topic','3.6','Unsupported opportunity',
...
test('deployment helpers validate the supplied release version instead of a hard-coded app version', async t => {
  ...
  assert.match(resume, /result\['schema'\]==72/);
  assert.match(resume, /h\.contentStrategy\.version!=="3\.6"/);
  assert.doesNotMatch(resume, /h\.contentStrategy\.version!=="3\.5"/);
  assert.match(resume, /MAX\(version\).*==72/);
});
