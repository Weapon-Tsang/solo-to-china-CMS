...
test("migrations 20 and 21 preserve populated legacy Sources and their evidence assets", async (t) => {
  ...
  const source = fs.readFileSync(dbModulePath, "utf8")
    .replace(/^  if \(current < (\d+)\).*$/gm, (line, version) => Number(version) >= 20 ? "" : line);
  ...
  try {
    assert.ok(upgraded.prepare("SELECT MAX(version) AS version FROM schema_migrations").get().version >= 20);
