...
test("migration 21 preserves v20 file evidence and extends Source storage for video", async (t) => {
  ...
  const source = fs.readFileSync(dbModulePath, "utf8")
    .replace(/^  if \(current < (\d+)\).*$/gm, (line, version) => Number(version) >= 21 ? "" : line);
  ...
  try {
    assert.ok(upgraded.prepare("SELECT MAX(version) AS version FROM schema_migrations").get().version >= 21);
