...
export function loadConfig(env = process.env) {
  ...
  return {
    ...
    deepseek: {
      ...
      imageBatchSize: Math.min(8, Math.max(4, integer(env.AI_IMAGE_BATCH_SIZE || env.DEEPSEEK_MAX_IMAGES, 8))),
    ...
    extraction: {
      ...
      mediaImageBatchSize: Math.min(8,Math.max(4,integer(env.MEDIA_EXTRACTION_BATCH_SIZE,8))),
