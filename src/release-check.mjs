...
if (packageJson.version !== "1.0.0" || manifest.version !== "1.0.0") {
  throw new Error(`Release versions must both be 1.0.0 (package=${packageJson.version}, extension=${manifest.version}).`);
}
...
try {
  ...
  try {
    ...
    if (versions.join(",") !== "1,2,3,4,5") throw new Error(`Unexpected migration chain: ${versions.join(",")}`);
...
console.log("Release check passed: version alignment, migrations 1-5, and SQLite integrity.");
