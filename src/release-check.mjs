...
try {
  ...
  try {
    ...
    if (versions.join(",") !== Array.from({ length: 33 }, (_, index) => index + 1).join(",")) throw new Error(`Unexpected migration chain: ${versions.join(",")}`);
...
console.log(`Release check passed: app version alignment, Content Strategy ${CONTENT_STRATEGY.version} governance, migrations 1-33, and SQLite integrity.`);
