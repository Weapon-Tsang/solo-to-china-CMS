...
try {
  ...
  try {
    ...
    if (versions.join(",") !== Array.from({ length: 32 }, (_, index) => index + 1).join(",")) throw new Error(`Unexpected migration chain: ${versions.join(",")}`);
...
console.log(`Release check passed: app version alignment, Content Strategy ${CONTENT_STRATEGY.version} governance, migrations 1-32, and SQLite integrity.`);
