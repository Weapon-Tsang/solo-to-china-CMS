...
function DraftDetail({ item, health, actionBusy, onAction, onClose }) {
  ...
  return <>
    ...
    <div className="mb-4 flex flex-wrap items-center gap-2"><StatusPill status={draft.status} />{publication?.remote_status === "publish" && <Badge variant="success">WordPress 已发布</Badge>}{publication?.remote_status === "draft" && <Badge variant="secondary">WordPress 草稿</Badge>}<Badge>质量审核 {review ? `${Math.round(review.score)} / 100` : "待处理"}</Badge>{review?.passed && health?.wordpressConfigured && draft.status === "ready_for_wordpress" && <Button size="sm" disabled={actionBusy} onClick={push}><Send /> 发送到 WordPress 草稿箱</Button>}<FinalPreviewAction draftId={draft.id} available={Boolean(publication?.preview_url)} complete={Boolean(composition?.current && publishComposition?.status === "delivered")} />{publication?.edit_url && <Button size="sm" variant="outline" asChild><a href={publication.edit_url} target="_blank" rel="noreferrer"><ExternalLink />在 WordPress 编辑</a></Button>}</div>
