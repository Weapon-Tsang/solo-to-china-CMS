...
function ProductionDetail({ item, health, actionBusy, onAction, onClose }) {
  ...
  return <>
    ...
    {state.latest_error?.code === "DESTINATION_TOPIC_MISMATCH" && <DetailCard title="更正目的地归属" className="mb-3 border-amber-200"><p>该错误不能靠重复重试解决。更正后系统会重新计算证据范围，并要求重新确认生产范围。</p><ContentRecovery candidateId={item.candidate_id} onAction={onAction} actionBusy={actionBusy} /></DetailCard>}
...
function modelExecutionLabel(value) {
  return ({ confirmed:"已确认执行", rejected_before_generation:"接口在生成前拒绝", not_requested:"未发起", unknown:"请求已发送，执行情况未确认" })[value] || "未确认";
}
