...
function ProductionDetail({ item, health, actionBusy, onAction, onClose }) {
  ...
  return <>
    ...
    {state.latest_error && <DetailCard title="准确失败原因" className="mb-3 border-rose-200 bg-rose-50/40"><p><strong>失败阶段：</strong>{state.current_stage_label}</p><p>{state.latest_error.reason}</p><details className="mt-2 rounded-lg border border-rose-100 bg-white/70 p-2"><summary className="cursor-pointer font-medium text-slate-700">技术详情</summary><dl className="mt-2 grid gap-1 break-all text-[10px] text-slate-600"><div>code: {state.latest_error.code || "—"}</div><div>job: {state.latest_error.job_id || "—"}</div><div>request: {state.latest_error.request_id || "—"}</div><div>attempt: {state.latest_error.attempt ?? "—"}</div><div>class: {state.latest_error.failure_class || "—"}</div><div>provider/model: {state.latest_error.provider || "—"} / {state.latest_error.model || "—"}</div><div>模型调用：{state.latest_error.model_called ? "是" : "否"}</div></dl></details></DetailCard>}
    ...
    <DetailCard title="生产时间线" className="mb-3"><ol className="space-y-2">{(state.timeline || []).map((step) => <li key={step.key} className="flex min-w-0 items-start justify-between gap-3 rounded-lg border border-slate-100 px-3 py-2"><div className="min-w-0"><strong>{step.label}</strong><small>{step.dependencies?.length ? `已有 ${step.dependencies.length} 个前置依赖` : "生产入口"}{step.reused ? " · 已复用持久化产物" : ""}</small></div><StatusPill status={step.status} /></li>)}</ol><p className="mt-3">{state.stage_status === "failed" ? `恢复目标：重新执行${state.recovery_target_label || state.current_stage_label}` : `下一步骤：${state.next_stage_label || "无"}`}</p><details className="mt-3 rounded-lg bg-slate-50 p-2"><summary className="cursor-pointer font-medium">展开内部 stage key</summary><ul className="mt-2 break-all text-[10px]">{(state.timeline || []).map((step)=><li key={step.key}>{step.label}: {step.key} · dependencies [{(step.dependencies || []).join(", ")}]</li>)}</ul></details></DetailCard>
