...
function SourceDetail({ source, actionBusy, onAction, onClose }) {
  ...
  const reviewEvidence = (decision, authorityLevel) => onAction(`/api/sources/${source.id}/evidence-review`, {
    ...
    body: JSON.stringify({ decision, authorityLevel, note: "Reviewed in source detail" }),
  }, decision === "verified" ? "Evidence verification saved; affected knowledge and opportunities are being rebuilt." : "Evidence trust state updated.");
  ...
  return <>
    ...
    <div className="mb-4 flex flex-wrap items-center gap-2">{originalUrl && <Button variant="secondary" size="sm" asChild><a href={originalUrl} target="_blank" rel="noreferrer"><ExternalLink /> Open original</a></Button>}<Button size="sm" disabled={actionBusy} onClick={retry}><RefreshCw className={cn(actionBusy && "animate-spin")} /> Re-run extraction</Button><Button size="sm" variant="secondary" disabled={actionBusy} onClick={() => reviewEvidence("verified", 1)}><CheckCircle2 /> Verify as official</Button><Button size="sm" variant="outline" disabled={actionBusy} onClick={() => reviewEvidence("unverified", 4)}>Mark unverified</Button><StatusPill status={source.status} /><Badge variant={source.verified_at ? "success" : "warning"}>Authority L{source.authority_level || 4} · {source.verified_at ? "verified" : "unverified"}</Badge></div>
