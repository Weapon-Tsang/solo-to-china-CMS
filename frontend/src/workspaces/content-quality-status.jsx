...
export function ContentQualityStatus({ operation, actionBusy = false, onRetry = null, onAction = null, compact = false }) {
  ...
  const previewCancel = async () => {
    ...
    setCancelPreview({ ...preview, executeEndpoint: action.executeEndpoint });
  };
  const confirmCancel = async () => {
    if (!cancelPreview) return;
    await onAction(cancelPreview.executeEndpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ previewId: cancelPreview.id }) }, "内容任务已取消，历史产物已保留");
  ...
  return <Card className="mb-3 border-slate-200 p-4 shadow-none">
    ...
    <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50/60 p-3 text-[10px] text-blue-900"><strong>下一步：</strong>{operation.nextAction?.label} · {operation.nextAction?.reason}<br /><strong>预计还需：</strong>{label(operation.estimatedAdditionalCalls?.status)} {operation.estimatedAdditionalCalls?.stages?.map(label).join("、")}</div>
    {onRetry && operation.retry && <Button className="mt-3" size="sm" variant="secondary" disabled={actionBusy} onClick={() => onRetry(operation.retry)}>只重试失败阶段：{label(operation.retry.stage)}</Button>}
    <div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={actionBusy} onClick={compare}>比较修订</Button>{onAction && <Button size="sm" variant="outline" disabled={actionBusy} onClick={previewCancel}>预览并取消</Button>}<span className="self-center text-[10px] text-slate-500">需要补证据时请到“来源”；需要修改正文时请使用上方处理入口。</span></div>
    {inspection && <p className="mt-2 rounded-md bg-slate-50 px-2 py-1.5 text-[10px] text-slate-600">{inspection}</p>}
    <ConfirmAction open={Boolean(cancelPreview)} onOpenChange={(open) => !open && setCancelPreview(null)} title="确认取消这篇内容的进行中任务？" description="只停止仍在排队或运行的步骤，已经保存的内容不会删除。" detail={`将停止：${cancelPreview?.affectedJobs?.map((item) => label(item.type)).join("、") || "当前没有排队任务"}。来源、证据、稿件修订和商业记录都会保留。`} confirmLabel="确认取消" destructive busy={actionBusy} onConfirm={confirmCancel} />
  </Card>;
}
