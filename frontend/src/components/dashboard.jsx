...
function MetricGroup({ icon: Icon, tone, eyebrow, title, value, unit, detail, stats }) {
  ...
  return <Card className={cn("group relative min-w-0 overflow-hidden border bg-gradient-to-br p-2.5 shadow-sm transition duration-200 sm:p-3 sm:hover:-translate-y-0.5 sm:hover:shadow-md", tones[tone])}>
    ...
    <div className="relative mt-2 hidden gap-1.5 sm:flex">{stats.map(([statValue, statLabel]) => <div key={statLabel} className="min-w-0 rounded-md bg-white/55 px-1.5 py-1"><span className="mr-1 text-xs font-semibold text-slate-900 tabular-nums">{statValue ?? 0}</span><span className="text-[9px] text-slate-500">{statLabel}</span></div>)}</div>
  </Card>;
}
